...
/* Will create a mix between oil and foam(lerp), depending on the distance between the center of the stain and the pixel
* Will only happen if pixel is in a stain area(if not, isInsideStainArea will be 0 and centerOfOilStain will be 0,0,0
* will return (0,0,0) if pixel is outside any stains, and Ceto_OilTint if it's within the inner circle
*/
inline float3 PossibleOilFoamMix(int pixelIsInsideStainArea, float3 centerOfCurrentOilStain, float3 pixelWorldPosition, fixed foamAmount)
{
	//if pixel is not inside any stain area, then color will be entirely based in FoamTint
	if(pixelIsInsideStainArea <=0) return Ceto_FoamTint*foamAmount;


	//else, it's inside oil stain area. Now to see if it's within the inner circle or between inner and outer circles
	float distToCenter = distance(centerOfCurrentOilStain, pixelWorldPosition);

	if (distToCenter<oilStainMinimumRadius) return Ceto_OilTint; //based entirely on OilTint

	//finally, a mix between oil and foam
	return lerp(Ceto_OilTint, Ceto_FoamTint, (distToCenter-oilStainMinimumRadius)/(oilStainMaximumRadius-oilStainMinimumRadius) )*foamAmount;
			//lerp between oil and foam, going from 0 in minRadius to 1 in maxRadius
...
* Spectrum foam in x, overlay foam in y.
* 
* UPDATE:MODDED IN 21/03/2017
* Now it also calculates whether the pixel is inside an oil spill zone, and if it is, tint it with the Oil Tint instead of the Foam Tint 
*/
inline fixed3 AddFoamColor(fixed foamAmount, fixed3 oceanCol, float3 worldPosition)
{
	...
	//####################################################################### proto mods ##############################################################################
	//will hold the number of stain area(s) this particular pixel is in(if any)
	int isInsideStainArea = 0;
	float3 closestOilCenter = float3(100000,100000,100000);//if the pixel is inside an oil stain area, we will need the center of that area


	for(int i = 0; i<numberOfOilStains;i++)
	{
		//in other words, if the pixel is within a distance from the current stain (max_Accepted_Oil_Distance) ....
		//We add 1 to the variable, to say that the pixel is inside a(nother) stain area
		//isInsideStainArea+=distance(worldPosition,oilCenterPositions[i])<=oilStainMaximumRadius;
		if (distance(worldPosition,oilCenterPositions[i])<=oilStainMaximumRadius && (distance(worldPosition,closestOilCenter) > distance(worldPosition,oilCenterPositions[i])) )
		{
			isInsideStainArea += 1;
			closestOilCenter = oilCenterPositions[i];
			//break;
		}
	}

	//We then calculate foam color based on the previous calculation...
	//If the pixel is outside EVERY oil stain area, it will be tinted with the normal foam color.
	//However, if it's within ANY oil stain area, it will be tinted with the oil tint.
	//(TODO: do a fade out on the oil tint starting from the center(or from a minimum radius...
	//... so the transition between oil and foam tints is smoother along the limits of the border)

	fixed3 foamCol = PossibleOilFoamMix(isInsideStainArea, closestOilCenter, worldPosition, foamAmount) * exp(-Ceto_AbsCof.rgb * (1.0 - foamAmount) * 1.0);
