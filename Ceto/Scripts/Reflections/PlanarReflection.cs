...
namespace Ceto
{
	...
	public class PlanarReflection : OceanComponent
    {
		...
		//#################################################################################################################################################################################################################
		//#################################################################################################################################################################################################################
		//##################################################### Mudanças no Script - Começo ###################################################################################################################################################
		string[] dados = new string[30];

		void pegaDados()
		{
			if (dados [25] == "True")
				skyboxInReflection = true;
			else
				skyboxInReflection = false;
			if (dados [26] == "True")
				fogInReflection = true;
			else
				fogInReflection = false;
			blurIterations = int.Parse (dados [27]);
			reflectionIntensity = float.Parse (dados [28]);
			reflectionDistortion = float.Parse (dados [29]);
		...
		public void RenderReflection(GameObject go)
		{
            ...
            try
            {
				...
				if(RenderReflectionCustom != null)
				{
                    ...
                    CreateRenderTarget(data.reflection, cam.pixelWidth, cam.pixelHeight, cam.hdr, false, data.settings);
				...
				else
				{
                    ...
                    CreateRenderTarget(data.reflection, cam.pixelWidth, cam.pixelHeight, cam.hdr, cam.stereoEnabled, data.settings);
