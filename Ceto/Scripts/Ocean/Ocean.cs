...
namespace Ceto
{
    ...
    public class Ocean : MonoBehaviour, ICoroutine
    {
		...
		/// The oil tint.
		/// </summary>
		public Color oilTint = Color.black;//proto mod
		...
		void pegaDados()
		{
			level = float.Parse (dados [0]);
			windDir = float.Parse (dados [1]);
			specularRoughness = float.Parse (dados [2]);
			specularIntensity = float.Parse (dados [3]);
			fresnelPower = float.Parse (dados [4]);
			minFresnel = float.Parse (dados [5]);
			foamIntensity = float.Parse (dados [6]);
        ...
        void Start()
        {
			...
			try
			{
				...
				Shader.SetGlobalColor("Ceto_FoamTint", Color.white);//Brian here; add "Ceto_OilTint" after this?
				Shader.SetGlobalColor("Ceto_OilTint", Color.black);
